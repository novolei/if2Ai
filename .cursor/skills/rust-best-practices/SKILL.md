...
按需阅读 **1～2 个** chapter 文件即可；**不要**在同一 turn 内并行读完所有章节（token 浪费）。按任务选读：

| 任务 | 优先读 |
|------|--------|
| 借用/clone/迭代器 | chapter_01 |
| clippy / lint 配置 | chapter_02 |
| 性能 / 克隆 / 堆栈 | chapter_03 |
| Result / thiserror / anyhow | chapter_04 |
| 测试命名 / snapshot | chapter_05 |
| 泛型 / dyn Trait | chapter_06 |
| typestate | chapter_07 |
| 注释 vs rustdoc | chapter_08 |
| Send/Sync / 指针 | chapter_09 |

完整章节索引（仅在需要深入时打开对应文件）：
