...
>
> 最后更新: 2026-04-21（M4 doc reconciliation closeout + **M5 closeout**：
> M4 后端治理链 m4.1 ~ m4.5 / m4.7 / m4.8 已 done；m4.6 前端 governance UI 显式延后；
> M5 全部 8 个 slice 已 done — trajectory scoring / failure clustering / 结构化 reflection /
> registry + history / compare+suite eval / promotion gate（PromotionBasis 默认 RequireBoth）+
> rollback + singleton-active supersede / diagnostics 前端 / safety integration test 全部到位；
> production active flip 通过 ActiveStrategyOverlayResolver 单 hook 接入 prompt planner，
> 不重写 commands/agent.rs 主链；M5 完成后的 enhancement 见 phase-m5-yaml notes 段。
> M0 产物清单见 §6.1）
...
- `M4` 后端治理链已完成（backend_complete_frontend_deferred 即满足）
- compare / gate / recommendation 已具备基础能力（见下方代码现实）
- 已阅读 `phase-m5-executor-runbook.md` 中的 “M5 readiness calibration” 段

#### M5 入场时可直接复用的输入契约（已实现）

| 契约                  | 模块 / IPC                                                                                                                          |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `HarnessRunReport`    | `src-tauri/src/modules/harness/run_report.rs` ＋ `harness_finalize_run` / `harness_finalize_and_rotate_run` / `harness_load_report` |
...
以下被 M4 显式延后或视为后续 enhancement，M5 slice 不允许默认它们存在：

1. governance UI（前端 dashboard / 治理 store / settings 治理 section）—— m4.6 deferred。
2. auto-promote / auto-rollback automation —— 不在 M4 范围。
3. per-corpus-tier weighted policy —— 当前 `GatePolicy` 单一默认 conservative。
4. closed-set evidence_ref scheme —— 现仍是 free-form 字符串。
5. 五层语料 YAML 全量填充 —— 契约 + 聚合 + IPC 已就绪，YAML 内容补齐视为后续 enhancement。

如 M5 需要任何上述能力，**必须** stop 并要求开新的 follow-on phase（例如 `phase-m4f-governance-dashboard-frontend` 或 `phase-m4g-gate-policy-enhancement`），不允许 M5 slice 顺手做。
...
### M4 退出条件（reconciliation 后的真实判定口径）

> 真实状态：**backend_complete_frontend_deferred**（2026-04-21）。
> 治理契约层级：`HarnessRunReport` / `BaselineVsCandidate` / `RegressionCorpus` / `SuiteReport` / `Recommendation` 五者并列；`Recommendation` 不内嵌进 `HarnessRunReport`。

后端必备项（已全部满足，是 M5 入场的硬条件）：

1. `prepare_step_execution` 进入真实链路（m4.1 done）
2. `HarnessRunReport` / `EvidenceBundle` / graders / `BlockingFailure` 存在（m4.2 / m4.3 / m4.4 done）
3. `BaselineVsCandidate` compare 契约 + IPC 存在（m4.5 done）
4. `RegressionCorpus` / `SuiteReport` 契约 + IPC 存在（m4.7 done；YAML 语料填充视为 enhancement，不阻塞）
5. `GatePolicy` / `Recommendation` 存在且默认 conservative（m4.8 done）

显式延后项（不阻塞 M5，但要求执行者不要把它当 M5 顺带工作）：

- m4.6 前端 governance dashboard / 治理 store / settings 治理 section
- auto-promote / auto-rollback automation
- per-corpus-tier weighted policy
- closed-set evidence_ref scheme

### M5 退出条件（全部满足，2026-04-21 closeout）

1. candidate strategy registry 存在 — ✓（含 history 栈 + StrategyDefinition + SupersedeRecord）
2. reflection -> candidate -> compare -> promote/hold/reject 闭环成立 — ✓
   （trajectory_score → failure_clustering → reflection_generator → register →
   evaluate_candidate / evaluate_candidate_against_suite → PromotionGate(RequireBoth)
   → mark_promoted → activate）
3. rollback path 存在 — ✓（typed audit chain + singleton-active 自动 supersede + 跨 store target validation）

加分项 / closeout 实际成果：
- production active flip 真实闭环：StrategyDefinition → ActiveStrategyOverlayResolver →
  PromptPlan.ActiveStrategyOverlay 块（turn_service.rs 单 hook）
- 前端 StrategyDiagnosticsPage 接入真实 IPC 显示 candidate / active / rollback / supersede
- 跨链路 integration test (`tests/m5_self_evolution_integration.rs`) 验证全链路 + 错误推广拦截
