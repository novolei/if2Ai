...
>
> 最后更新: 2026-04-21（M4 reconciliation：m4.3 / m4.4 / m4.5 / m4.7 全部 done；本计划留作回看 + contract 叙事校准）

## 0. Reconciliation 概要（必读）

- `m4.3` graders + blocker rules: done（`src-tauri/src/modules/harness/graders/`、`gate.rs::GatePolicy::default`）。blocker / recommendation 规则集中在 gate.rs，不要再期望它们写在 graders 模块里。
- `m4.4` evidence trace wiring: done（`EvidenceBundle` 实接 prepare_step / execution_mode / memory / permission / stream_error）。
- `m4.5` compare flow: done（`compare.rs::compare_reports` + `BaselineVsCandidate`）。**compare 不输出 Recommendation**；它输出 diff，gate 才输出 Recommendation。
- `m4.7` corpus + suite aggregation: done（`corpus.rs` + `suite_report.rs`）。仓库内五层 YAML 语料填充作为后续 enhancement，不阻塞 M5。
- 治理契约层级：`HarnessRunReport`（run）／`BaselineVsCandidate`（compare）／`SuiteReport`（suite 聚合）／`Recommendation`（gate 决策）四者并列。
...
### 必须落实（已落地于 `compare.rs`）

1. baseline 与 candidate 跑同一任务集（多任务集合走 m4.7 `SuiteReport`）
2. 输出 diff summary（`AggregateDiff` / `EvidenceSummaryDiff`）
3. 输出 blocker diff（`BlockerDiff`）
4. **不在 compare 输出 recommendation**——recommendation 由 m4.8 gate 在消费 `BaselineVsCandidate` / `SuiteReport` 后渲染（`gate.rs::evaluate_compare` / `evaluate_suite`）。
