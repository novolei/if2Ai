 ...
 * │  └────────────────────────────────────────────────────────────┘  │
 * │ Meta bar                                                          │
 * │  [cpu] GPT-5.4-Mini ▾  |  [地球] 中 ▾          完全访问权限  │  │
 * │                                          [branch] feature/… ▾   │  │
 * └───────────────────────────────────────────────────────────────────┘
 *
 * States
 * • empty    : send button bg-muted, text-muted-foreground, cursor-not-allowed
 * • has-text : send button bg-jade, text-primary-foreground, hover:opacity-90 active:scale-95
 * • focused  : container border-ring + shadow-ring-jade
 * • disabled : entire composer opacity-60, no interactions
 *
 * Padding / Spacing (input box)
 * • Container: px-4 py-2.5 rounded-xl
 * • gap-3 (12px) between [+] / text / mic / send
 * • send button: 28×28px rounded-lg
 * • [+] and mic: 24×24px rounded-md
 *
 * Meta bar
 * • mt-2 px-1 from input box
 * • text-[11px] text-muted-foreground
 * • gap-3 between meta items
 * • divider: h-3 w-px bg-border
 *
 * Colors
 * • box bg          : bg-card
 * • box border      : border-input → border-ring (focus)
 * • box shadow      : shadow-token-sm → shadow-ring-jade (focus)
 * • placeholder     : text-muted-foreground
 * • [+] / mic hover : text-jade
 * • send (empty)    : bg-muted, text-muted-foreground
 * • send (active)   : bg-jade, text-primary-foreground
 */
export default function InputComposer({
  ...
  const handleSubmit = () => {
    ...
    console.log("InputComposer: submit", current);
    onSubmit(current);
    setInternal("");
    onChange("");
  };
