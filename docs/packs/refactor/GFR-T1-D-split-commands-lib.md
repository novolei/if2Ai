# GFR-T1-D: Split `modules/commands/lib.rs` (2667 LOC)

## Status
- State: `pending`
- Charter: [../CHARTER.md](../CHARTER.md)
- Predecessor: `GFR-T1-C` must be `done` first
...
按 GFR roadmap 第 T1-D 步：把 source 中列出的代码段以**纯位移**方式搬到 destination 模块；零业务变更。
...
- command registry
- plugin bridge（依赖 `plugins::*`）
- runtime bridge（依赖 `runtime::compact_session`）
...
- `modules/commands/{registry,plugin_bridge,runtime_bridge,mod}.rs`
...
- ❌ 不改 `CommandSource` enum
- ❌ 不改 manifest entry 名规则
