# GFR-009: Extract tool call summary / glyph / status helpers

## Status
- State: `pending`
- Charter: [../CHARTER.md](../CHARTER.md)
- Predecessor: `GFR-008` must be `done` first
...
按 GFR roadmap 第 009 步：把 source 中列出的代码段以**纯位移**方式搬到 destination 模块；零业务变更。

---

## Source (candidate ranges — re-verify on activation)

- `src/components/ui/chat-ui.tsx` (≈3955–4286)
-   - `pickToolHeadline` / `buildToolDetailLines` / `buildToolCommandResultLine`
-   - `normalizeToolLabel` / `renderInlineToolSummary` / `summarizeToolResult`
-   - json + 文本截断工具
...
- 新建 `src/modules/tool-projection/`：`headline.ts`、`details.ts`、`summary.ts`、`label.ts`、`truncate.ts`、`index.ts`
