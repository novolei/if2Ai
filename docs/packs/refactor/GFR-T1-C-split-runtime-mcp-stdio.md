# GFR-T1-C: Split `modules/runtime/mcp_stdio.rs` (1725 LOC)

## Status
- State: `pending`
- Charter: [../CHARTER.md](../CHARTER.md)
- Predecessor: `GFR-T1-B` must be `done` first
...
按 GFR roadmap 第 T1-C 步：把 source 中列出的代码段以**纯位移**方式搬到 destination 模块；零业务变更。

---

## Source (candidate ranges — re-verify on activation)

- JSON-RPC framing（`JsonRpcId` / `JsonRpcRequest` / `JsonRpcResponse`）
- stdio transport（`Child` / `ChildStdin` / `ChildStdout`）
...
- `modules/runtime/mcp/{rpc,transport,bootstrap,mod}.rs`
- 原文件退化为 shim
