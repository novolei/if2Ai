# GFR-T1-I: Split `commands/tts.rs` (1368 LOC)

## Status
- State: `pending`
- Charter: [../CHARTER.md](../CHARTER.md)
- Predecessor: `GFR-T1-H` must be `done` first
...
按 GFR roadmap 第 T1-I 步：把 source 中列出的代码段以**纯位移**方式搬到 destination 模块；零业务变更。
...
- 12 个 Tauri commands
- `StreamingJobManager` / `WarmupManager` 编排
- voice registry / demo 解析
...
- `modules/tts/{health,warmup,synthesize,stream,demo,voices}/` 内化逻辑
- `commands/tts.rs` 退化为 thin Tauri entry（每个 command ≤ 30 行）
