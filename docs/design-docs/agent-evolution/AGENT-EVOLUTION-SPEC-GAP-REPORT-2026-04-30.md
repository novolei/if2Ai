...
| Pack                                   | REGISTRY 当前 | 实测              | 证据                                                                                                                                                                                                                                                                                                           |
| -------------------------------------- | ------------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **DW-001** self-edit scanner spawn     | active        | ✅ **done** (Iteration 7) | 真化于 commits `50808e1` / `af3a890` / `9e36e72`：`spawn_self_edit_scanner_interval` 现接 `ChatProviderUtilityLlm` + `FastEmbedProvider`（fallback 至 `ConstFallbackEmbedder`）+ `load_scanner_tick_input`（从 `~/.if2ai/harness/runs/` 加载最近 100 份 `HarnessRunReport` 推导 `failure_rate` / `sample_size`）+ `ScannerStageState` 跨 tick 持久化。详见 §13                                                                                                          |
