...
function RailIcon({
  ...
  return (
    <Icon className={cn("h-3.5 w-3.5 shrink-0", className)} strokeWidth={1.5} />
  );
}
...
function humanizeIdentityToken(value: string): string {
  return value
    .split(/[-_]/g)
    .filter(Boolean)
...
function resolveIdentityBadge(input: {
  soulId?: string | null;
  personaId?: string | null;
}): { label: string; tone: string } | null {
  if (input.personaId) {
    return {
      label: humanizeIdentityToken(input.personaId),
      tone: "border-amber-200/70 bg-amber-50 text-amber-800",
    };
  }
  if (input.soulId) {
    return {
      label: humanizeIdentityToken(input.soulId),
    ...
    if (sortOrder === "alpha") {
      ...
      list = [...list].sort((a, b) => {
        const latestA = (projectSessions[a.id] ?? []).reduce(
          (t, s) => Math.max(t, +new Date(s.updated_at)),
          0,
        );
      ...
      {pinnedSessions.length > 0 && (
        <div className="shrink-0 border-t border-black/[0.06] px-3 pt-2 pb-1.5">
          ...
          <div className="mb-1 flex items-center gap-1.5 px-1">
            <Pin
              className="size-2.5 shrink-0 rotate-45 text-muted-foreground/35"
              strokeWidth={2}
            />
          ...
          <div className="flex flex-col">
            {pinnedSessions.map((item) => (
              <PinnedItem
                ...
                onUnpin={() =>
                  void onTogglePinSession(item.projectId, item.sessionId, true)
                }
              />
            ))}
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-0">
            ...
            <Button
              ...
              onClick={() =>
                setSortOrder((o) => (o === "recent" ? "alpha" : "recent"))
              }
              className={cn(
  ...
  return (
    ...
    >
      ...
      {identityBadge ? (
        <span
          className={cn(
            "max-w-[120px] truncate rounded-full border px-1.5 py-0.5 text-[9px] font-medium leading-none",
            identityBadge.tone,
          )}
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 items-center gap-1.5">
        ...
        {identityBadge ? (
          <span
            className={cn(
              "max-w-[104px] shrink-0 truncate rounded-full border px-1.5 py-0.5 text-[9px] font-medium leading-none opacity-85",
              identityBadge.tone,
            )}
            title={identityBadge.label}
          >
      ...
      <div className="relative flex h-full shrink-0 items-center">
        ...
        <DropdownMenu>
          ...
          >
            ...
            >
              <Pin
                className="size-3.5 shrink-0 rotate-45 text-muted-foreground/60"
                strokeWidth={1.5}
              />
    ...
    <DropdownMenu>
      ...
      >
        ...
        <DropdownMenuItem
          className="h-8 rounded-lg px-2.5 text-[12.5px] font-medium text-foreground/82 transition-colors hover:bg-accent focus:bg-accent focus:text-foreground/90"
          onSelect={() => {
            void Promise.resolve(onCreatePermanentWorktree(project.id)).catch(
              (error) => {
