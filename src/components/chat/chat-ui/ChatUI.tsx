/**
 * GF-01 PR-09 — DT-03 final cutover.
 *
 * Orchestrator shell for the chat surface. Composes transcript +
 * composer + sidebars + panels and owns the high-level render
 * branches (preview-focus split column, project rail visibility,
 * scroll-to-bottom guard).
 *
 * DT-03 contract: `ChatUI` no longer accepts a `messages` prop.
 * The component subscribes to the runtime projection store
 * (`useRuntimeProjectionSelector` + `currentSessionId` guard) and
 * the chat-store user-anchor history (`useConversation`), then
 * folds them through `projectConversationMessagesFromRuns` exactly
 * once. ER-03's atomic `swapSession` ensures the snapshot's
 * `currentSessionId` is the authoritative scope guard.
 *
 * Note (partial cutover): the persisted user-anchor history still
 * lives in the chat-store conversation slice (loaded by
 * `loadConversationHistory`) and is read here via
 * `useConversation(sessionId)`. The shell merges that with the
 * projection's runs the same way `App.tsx` previously did, so
 * historical sessions render identically post-cutover.
 */
...
/**
 * DT-03 contract: NO `messages` field. ChatUI reads the conversation
 * history from `useConversation(sessionId)` (chat-store) and the
 * runtime projection (`useRuntimeProjectionSelector`) internally.
 */
export interface ChatUIProps {
  ...
  isLoading?: boolean
  sessionTitle?: string
  /** Active session id (also the projection scope key + chat-store key). */
  sessionId?: string
  projectLabel?: string
...
export function ChatUI({
  ...
  // DT-03 — ChatUI reads its own messages from the projection store
  // (runs + currentSessionId guard) merged with the chat-store
  // user-anchor history.
  const activeConv = useConversation(sessionId ?? null)
  const projectionRuns = useRuntimeProjectionSelector((s) => s.runs)
  const projectionCurrentSessionId = useRuntimeProjectionSelector(
    (s) => s.currentSessionId,
  )
  const messages = React.useMemo<Message[]>(() => {
    const baseMessages = activeConv?.messages ?? []
    const hasProjectionRunsForSession =
      sessionId &&
      Object.values(projectionRuns).some((run) => run.sessionId === sessionId)
    if (
      !hasProjectionRunsForSession &&
      baseMessages.some((msg) => msg.role !== 'user')
    ) {
    ...
    const filteredAnchors = baseMessages.filter(
      (msg) => !msg.content.includes('[resume_cursor]'),
    )
    // ER-03 scope guard: when the projection's currentSessionId no
    // longer matches our session prop, treat runs as empty so a
    // mid-swap render can't leak the previous session's stream.
    const scopedRuns =
      projectionCurrentSessionId === null ||
      projectionCurrentSessionId === sessionId
        ? projectionRuns
        : {}
    return projectConversationMessagesFromRuns(
      ...
      scopedRuns,
      sessionId,
    ).map((msg) => ({ ...msg, content: msg.content || ' ' })) as Message[]
  }, [activeConv?.messages, sessionId, projectionRuns, projectionCurrentSessionId])
  ...
  const [selectedPermissionMode, setSelectedPermissionMode] =
    React.useState<PermissionMode>(permissionModeProp)
  ...
  const handleModelChange: React.Dispatch<React.SetStateAction<string>> =
    onModelChangeProp ?? setSelectedModel
  const permissionModeValue =
    onPermissionModeChangeProp !== undefined ? permissionModeProp : selectedPermissionMode
  ...
  const navigateUpFolder = React.useCallback(async () => {
    setAtOverlay((prev) => {
      if (!prev) return null
      if (prev.breadcrumbs.length === 0) return null
      return prev
    })
