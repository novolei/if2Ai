...
interface TelemetryDrawerErrorBoundaryProps {
  children: ReactNode
  open: boolean
  onClose: () => void
}

interface TelemetryDrawerErrorBoundaryState {
  error: Error | null
}

class TelemetryDrawerErrorBoundary extends Component<
  TelemetryDrawerErrorBoundaryProps,
  TelemetryDrawerErrorBoundaryState
> {
  state: TelemetryDrawerErrorBoundaryState = { error: null }

  static getDerivedStateFromError(error: Error): TelemetryDrawerErrorBoundaryState {
    return { error }
  }

  render() {
    if (!this.props.open) return this.props.children
    ...
    return (
      <>
        ...
        <aside
          role="complementary"
          aria-label="开发者遥测面板"
          className="fixed right-0 top-0 z-50 flex h-full w-[26rem] flex-col border-l border-border/70 bg-background text-foreground shadow-[0_0_48px_rgba(15,23,42,0.10)] dark:shadow-[0_0_48px_rgba(0,0,0,0.34)]"
        >
          <div className="flex items-center justify-between border-b border-border/70 bg-muted/25 px-4 py-3">
            <div className="leading-tight">
              ...
              <div className="text-[10px] text-muted-foreground">Developer telemetry</div>
            </div>
            <button
              type="button"
              onClick={this.props.onClose}
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-5">
            <div className="mt-4 rounded-lg border border-destructive/20 bg-destructive/8 px-3 py-2 text-[11px] leading-5 text-destructive">
              Developer telemetry render failed: {this.state.error.message}
            </div>
          </div>
        </aside>
      </>
...
function getToolSuccessRate(telemetry: SessionTelemetry, toolName: string): number {
  const calls = recordValue(telemetry.tool_calls)[toolName] ?? 0
  if (calls === 0) return 0
  const successes = recordValue(telemetry.tool_successes)[toolName] ?? 0
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-1.5">
      ...
      <div className="text-right">
        ...
        {subValue && (
          <span className="ml-1.5 text-[10.5px] text-muted-foreground">{subValue}</span>
        )}
      </div>
    </div>
  )
...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="mb-1.5 mt-4 border-t border-border/70 pt-2.5 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
      {title}
    </div>
  )
}
...
const MEMORY_TIMELINE_META: Record<MemoryPromoEventName, MemoryTimelineMeta> = {
  ...
  memory_promotion_candidate: {
    ...
    bg: 'bg-violet-500/[0.08] border-violet-500/20',
    label: '候选晋升',
  },
}
...
export function TelemetryDrawer(props: TelemetryDrawerProps) {
  return (
    <TelemetryDrawerErrorBoundary
      key={`${props.sessionId ?? 'none'}:${props.open ? 'open' : 'closed'}`}
      open={props.open}
...
function TelemetryDrawerContent({
  sessionId,
  ...
  const executionMode = useRuntimeProjectionSelector((s) => s.executionMode)
  ...
  const isRecording = harness !== null &&
    sessionId !== null &&
    stringArrayValue(harness.active_recordings).includes(sessionId)
  ...
  const toolCalls = telemetry ? recordValue(telemetry.tool_calls) : {}
  const toolEntries = telemetry
    ? Object.entries(toolCalls).sort(([, a], [, b]) => b - a)
    : []

  const turnsCompleted = numberValue(telemetry?.turns_completed)
  const turnsSucceeded = numberValue(telemetry?.turns_succeeded)
  const llmCalls = numberValue(telemetry?.llm_calls)
  const inputTokensTotal = numberValue(telemetry?.input_tokens_total)
  const outputTokensTotal = numberValue(telemetry?.output_tokens_total)
  const reflectionCycles = numberValue(telemetry?.reflection_cycles)
  const reflectionInsightsTotal = numberValue(telemetry?.reflection_insights_total)
  const compactionEvents = numberValue(telemetry?.compaction_events)
  const permissionPrompts = numberValue(telemetry?.permission_prompts)
  const totalTurnDurationMs = numberValue(telemetry?.total_turn_duration_ms)
  const avgTurnMs = turnsCompleted > 0
    ? Math.round(totalTurnDurationMs / turnsCompleted)
    : 0
    ...
    <>
      ...
      <aside
        ...
        className={cn(
          ...
          'border-l border-border/70 bg-background text-foreground shadow-[0_0_48px_rgba(15,23,42,0.10)] dark:shadow-[0_0_48px_rgba(0,0,0,0.34)]',
          className,
        )}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border/70 bg-muted/25 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            {loading && (
              <RefreshCw className="ml-1 h-3 w-3 animate-spin text-muted-foreground" aria-label="加载中" />
            )}
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-5">
          ...
          {/* Harness status */}
          {harness && (
            <div className="mt-3 flex items-center justify-between rounded-lg border border-border/70 bg-muted/25 px-3 py-2">
              <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                <CircleDot
                  className={cn(
                    'h-3 w-3',
                    Boolean(harness.harness_enabled) ? 'text-status-success' : 'text-muted-foreground/70',
                  )}
                  aria-hidden
                />
                <span className="font-medium">
                  {Boolean(harness.harness_enabled) ? 'Harness 已启用' : 'Harness 未启用'}
                </span>
              </div>

              {Boolean(harness.harness_enabled) && sessionId && (
                <button
                  type="button"
          ...
          {telemetry && (
            <>
              ...
              <StatRow
                ...
                subValue={turnsSucceeded < turnsCompleted
                  ? `${turnsSucceeded} 成功`
                  : undefined}
              />
              <StatRow
              ...
              <StatRow
                icon={Database}
                label="输入"
                value={formatNumber(inputTokensTotal)}
              />
              <StatRow
                icon={Database}
                label="输出"
                value={formatNumber(outputTokensTotal)}
              />
              <StatRow
                icon={Database}
                label="合计"
                value={formatNumber(inputTokensTotal + outputTokensTotal)}
              />
              ...
              <StatRow
                icon={Lightbulb}
                label="反思周期"
                value={String(reflectionCycles)}
                subValue={`${reflectionInsightsTotal} 条洞见`}
              />
              <StatRow
                icon={Database}
                label="上下文压缩"
                value={String(compactionEvents)}
              />
              <StatRow
                icon={Activity}
                label="权限请求"
                value={String(permissionPrompts)}
              />

              {/* Last event */}
              {telemetry.last_event_at && (
                <div className="mt-3 rounded-md border border-border/70 bg-muted/25 px-2.5 py-1.5 text-[10px] text-muted-foreground">
          ...
          {executionMode ? (
            <>
              <SectionHeader title="Execution Decision" />
              <div className="mt-1.5 rounded-lg border border-border/70 bg-muted/25 px-3 py-2">
                <div className="mb-1.5 text-[11px] text-muted-foreground">
                  当前分类判定
                </div>
                <ExecutionModePill className="max-w-full" />
              </div>
            </>
          ) : null}
