...
export interface ContentRouterProps {
   ...
   * `AppSection` the chat / memory surfaces don't handle. The
   * narrowed type mirrors `SectionWorkspace` — `'chat'` and
   * `'memory'` never reach this callback. */
  sectionWorkspace: (props: {
    section: Exclude<AppSection, 'chat' | 'memory'>
    onBackToChat: () => void
  }) => ReactNode
}
...
export function ContentRouter({
  ...
  // Narrowed by the two `return`s above.
