...
/** `git worktree list` text. */
export async function gitWorktrees(cwd: string): Promise<string> {
  return invoke<string>("git_worktrees", { cwd });
}

/** Add a worktree at `target`.
 *  - `branch = undefined` → `git worktree add <target>`.
 *  - `branch = "feat/x"` and branch exists → checkout into worktree.
 *  - `branch = "feat/x"` and branch absent → `git worktree add <target> -b feat/x`. */
...
export async function gitRemoveWorktree(
  ...
  await invoke<void>("git_remove_worktree", { cwd, target });
}

/** `git worktree prune`. */
export async function gitPruneWorktrees(cwd: string): Promise<void> {
  await invoke<void>("git_prune_worktrees", { cwd });
}

/** Newly-registered project returned from
 *  [`gitCreateWorktreeProject`].  Mirrors `modules::projects::Project`
 *  (snake_case to match the rest of the project IPCs in `@/lib/tauri`). */
...
/** Combined "create worktree on disk → register as a project" call.
 *
 *  Replaces the manual two-step flow (`git_add_worktree` + manual
 *  refresh) so the side bar / project list update in a single round-
 *  trip.  Returns the new project so the front-end can switch the
 *  active session into the worktree without a second IPC. */
export async function gitCreateWorktreeProject(args: {
  ...
  return invoke<CreatedWorktreeProject>("git_create_worktree_project", {
    cwd: args.cwd,
    target: args.target,
