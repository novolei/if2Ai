...
import { startAgentStream, stopAgentStream, respondPermission } from './streaming.ts'
import {
  createSession,
  deleteSession,
  getSession,
...
describe('api/streaming — wire contract', () => {
  it('startAgentStream forwards to `start_agent_stream` with camelCase args', async () => {
    const { client, calls } = recordingClient({ start_agent_stream: 'stream-id-123' })
    setApiClient(client)
  ...
  it('stopAgentStream forwards to `stop_agent_stream` with the stream id', async () => {
    ...
    assert.deepEqual(calls, [{ command: 'stop_agent_stream', args: { streamId: 'stream-xyz' } }])
  })
...
describe('api/sessions — wire contract', () => {
  it('createSession forwards projectId + title', async () => {
    const { client, calls } = recordingClient({ create_session: { id: 's1', title: 't' } })
    setApiClient(client)
  ...
  it('getSession, renameSession, deleteSession, setSessionPinned each map to their command', async () => {
    ...
    assert.deepEqual(
      ...
      ['get_session', 'rename_session', 'delete_session', 'set_session_pinned'],
    )
  })
})
...
describe('api/projects — wire contract', () => {
  it('createProject / listProjects / pickFolderDialog / ensureDefaultWorkdir dispatch correctly', async () => {
    ...
    assert.deepEqual(
      calls.map((c) => c.command),
      ['create_project', 'list_projects', 'pick_folder_dialog', 'ensure_default_workdir'],
    )
