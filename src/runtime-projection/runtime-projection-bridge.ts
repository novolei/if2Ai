...
export function wireRuntimeProjectionListeners(
  ...
  const familyHandlers: FamilyHandler[] = [
    ...
    {
      // PR C-3 cut-over: memory lifecycle events on the canonical
      // `runtime_event` channel; the legacy `memory_event` listener
      // is gone.
      eventType: 'memory',
      family: 'lifecycle',
      handle: (envelope) => {
    ...
    {
      // PR C-3 cut-over: per-turn memory batch envelope. Replaces
      // the legacy `memory_after_turn` listener; per-decision
      // fan-out for the rolling ring is preserved inline.
      eventType: 'memory',
      family: 'after_turn',
      handle: (envelope) => {
