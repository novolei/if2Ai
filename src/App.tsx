            ...
            const unlisten = await handle.subscribe(
              (payload: StreamTokenPayload) => {
                ...
                if (payload.event_type === "text_delta" && payload.text) {
                  ...
                  payload.event_type === "thinking_delta" &&
                  payload.thinking
                ) {
                  ensureAssistantMessage();
                  ...
                  if (toolCallId) {
                    ...
                    setConversations((prev) => {
                      ...
                      const existingMessage =
                        existingIndex >= 0
                          ? currentConv.messages[existingIndex]
                          : undefined;
                      ...
                      const updatedToolMessage: Message = {
                        ...
                        toolCallId,
                        streamId:
                          payload.stream_id ?? existingMessage?.streamId,
                        toolName:
                          payload.tool_name ??
                          existingMessage?.toolName ??
                          "unknown",
                        toolArgs:
                          payload.tool_args ?? existingMessage?.toolArgs,
                        toolDurationMs:
                          payload.tool_duration_ms ??
                          existingMessage?.toolDurationMs,
                        isError:
                          nextStatus === "error" || existingMessage?.isError,
                        toolStatus: nextStatus,
                        effectiveWorkdir:
                          payload.effective_workdir ??
                          existingMessage?.effectiveWorkdir,
                        policyDecision:
                          memoryFields?.policyDecision ??
                          payload.policy_decision ??
                          existingMessage?.policyDecision,
                        memoryScope:
                          memoryFields?.memoryScope ??
                          existingMessage?.memoryScope,
                        memoryReasonCode:
                          memoryFields?.memoryReasonCode ??
                          existingMessage?.memoryReasonCode,
                        evidenceId:
                          payload.evidence_id ?? existingMessage?.evidenceId,
                        requestId:
                          payload.request_id ?? existingMessage?.requestId,
                        taskOutcome:
                          payload.task_outcome ?? existingMessage?.taskOutcome,
                        degradedReason:
                          payload.degraded_reason ??
                          existingMessage?.degradedReason,
                        resumeAvailable:
                          payload.resume_available ??
                          existingMessage?.resumeAvailable,
                        resumeCursor:
                          payload.resume_cursor ??
                          existingMessage?.resumeCursor,
                        disableAnimation: true,
                      };
                      const nextMessages =
                        existingIndex >= 0
                    ...
                    if (
                      ...
                      const nextTodos = extractTodosFromToolResult(
                        payload.tool_result,
                      );
                      if (nextTodos) {
                        setSessionTodos((prev) => ({
                            ...
                            if (msg.id === assistantMsgId) {
                              return {
                                ...
                                taskOutcome:
                                  ...
                                  "completed",
                                degradedReason:
                                  payload.degraded_reason ?? msg.degradedReason,
                                resumeAvailable:
                                  payload.resume_available ??
                                  msg.resumeAvailable ??
                                  false,
                                resumeCursor:
                                  payload.resume_cursor ?? msg.resumeCursor,
                                memoryContext:
                                  payload.memory_context ?? msg.memoryContext,
                                contextBudgetUsage:
                                  payload.context_budget_usage ??
    ...
    try {
      ...
      const unlisten = await handle.subscribe((payload: StreamTokenPayload) => {
        ...
        if (payload.event_type === "text_delta" && payload.text) {
          ...
          if (toolCallId) {
            ...
            setConversations((prev) => {
              ...
              const updatedToolMessage: Message = {
                ...
                toolName:
                  payload.tool_name ?? existingMessage?.toolName ?? "unknown",
                toolArgs: payload.tool_args ?? existingMessage?.toolArgs,
                toolDurationMs:
                  payload.tool_duration_ms ?? existingMessage?.toolDurationMs,
                ...
                policyDecision:
                  ...
                  existingMessage?.policyDecision,
                memoryScope:
                  memoryFields?.memoryScope ?? existingMessage?.memoryScope,
                memoryReasonCode:
                  memoryFields?.memoryReasonCode ??
                  existingMessage?.memoryReasonCode,
                ...
                taskOutcome:
                  payload.task_outcome ?? existingMessage?.taskOutcome,
                degradedReason:
                  payload.degraded_reason ?? existingMessage?.degradedReason,
                resumeAvailable:
                  payload.resume_available ?? existingMessage?.resumeAvailable,
                resumeCursor:
                  payload.resume_cursor ?? existingMessage?.resumeCursor,
                disableAnimation: true,
              };
              ...
              const nextMessages =
                existingIndex >= 0
                  ? currentConv.messages.map((msg, index) =>
                      index === existingIndex ? updatedToolMessage : msg,
                    )
                  : [...currentConv.messages, updatedToolMessage];
          ...
          payload.event_type === "final_text_override" &&
          payload.text
        ) {
          cancelScheduledAssistantFlush();
                  ...
                  messages: currentConv.messages.map((msg) => {
                    if (msg.id === assistantMsgId) {
                      return {
                        ...
                        thinkingTime: Date.now() - startTime,
                        taskOutcome:
                          streamTaskOutcome ?? msg.taskOutcome ?? "completed",
                        degradedReason:
                          streamDegradedReason ?? msg.degradedReason,
                        resumeAvailable:
                          streamResumeAvailable ?? msg.resumeAvailable ?? false,
                    ...
                    if (
                      ...
                      return {
                        ...
                        content:
                          msg.content ||
                          "stream completed before tool reached terminal state",
                      };
                    }
                    return msg;
                  }),
