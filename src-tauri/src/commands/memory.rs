...
// ─── Phase 8B.11 / T-UI-3 — session-summary listing for the
// MemoryNarrativeViewer (timeline of rolling summaries grouped by
// date).  Read-only; reuses the existing `SessionSummaryStore` plumbing
// installed on `AppState` from 8A.5.

/// Frontend-facing DTO for one [`crate::modules::memory::summary::schema::SessionSummaryRecord`]
/// row.  Mirrors the backend record one-to-one but flattens
/// [`SummarySource`] to a stable `"rolling"` / `"compact"` string so
/// the React layer can pattern-match without importing a Rust enum
/// shape.
///
...
///
/// `limit` caps the returned count (default 100, hard ceiling 500 so a
/// runaway frontend cannot fault the IPC channel by requesting tens of
/// thousands of rows).  `since_days` (optional) restricts to summaries
