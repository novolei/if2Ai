//! Safety layer for prompt injection defense (adapted from Steward `steward_safety`, MIT OR Apache-2.0).
//!
...
/// Default max tool output size (bytes) before truncation in the safety layer.
pub const DEFAULT_MAX_TOOL_OUTPUT_BYTES: usize = 512 * 1024;

static SHARED_SAFETY_LAYER: OnceLock<SafetyLayer> = OnceLock::new();

/// Process-global [`SafetyLayer`] (built once; avoids rebuilding regex-heavy leak detector per tool call).
#[must_use]
pub fn shared_safety_layer() -> &'static SafetyLayer {
    SHARED_SAFETY_LAYER.get_or_init(default_safety_layer)
}

/// Build a [`SafetyLayer`] from environment overrides (for tests or custom wiring).
#[must_use]
pub fn default_safety_layer() -> SafetyLayer {
    let injection_check_enabled = std::env::var("IF2AI_SAFETY_INJECTION_CHECK")
        .map(|v| v != "0")
        .unwrap_or(true);
    let max_output_length = std::env::var("IF2AI_SAFETY_MAX_TOOL_OUTPUT_BYTES")
        .ok()
...
impl SafetyLayer {
    ...
    pub fn scan_inbound_for_secrets(&self, input: &str) -> Option<String> {
        let warning = "Your message appears to contain a secret (API key, token, or credential). \
             ...
             To store provider credentials, use Settings → Providers (or onboarding), not the chat box.";
