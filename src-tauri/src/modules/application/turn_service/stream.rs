...
//!
//! File-size note: this module currently exceeds the CHARTER §5
//! 800-LOC backend hard limit because the spawned tool-loop
//! closure has not yet been refactored into smaller pieces.
//! MIG-001-d will internally split the closure body into sibling
//! `stream_*.rs` files; until then the file is added to the
//! REGISTRY tier-3 watchlist with that rationale so
//! `lint-architecture` does not error on the transient state.
...
impl TurnService {
    ...
    ///
    /// MIG-001-d will further refactor the spawned tool-loop body
    /// into smaller pieces inside this module; the public
    /// signature is final.
    ///
    /// # Errors
    ///
    ...
    pub async fn stream_turn(&self, request: StreamTurnRequest) -> Result<String, String> {
        ...
        let permission_senders = request.permission_senders.clone();
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                loop {
                    match stream.next_event().await {
                        Ok(Some(event)) => match event {
                            ApiStreamEvent::ContentBlockDelta(delta_event) => match delta_event
                                .delta
                            {
                                crate::modules::api::ContentBlockDelta::TextDelta { text } => {
                                    ...
                                    if token_count.is_multiple_of(SAVE_INTERVAL) {
                                        ...
                                        let _ =
                                            session_manager.save_session(&interim_session).await;
                                        tracing::debug!(
                                ...
                                crate::modules::api::ContentBlockDelta::ThinkingDelta {
                                    thinking,
                                } => {
                ...
                for (tool_id, tool_name, input_json) in pending_tool_uses.drain(..) {
                    ...
                    let permission_outcome = match remembered_decision {
                        ...
                        None => permission_policy.authorize(
                            &tool_name,
                            &input_json,
                            Some(&mut prompter),
                        ),
                    ...
                    if let crate::modules::runtime::permissions::PermissionOutcome::Deny {
                        reason,
                    } = &permission_outcome
                    {
                        tracing::warn!(
            ...
            if (stream_failed || user_visible_truth.task_outcome == "partial_success")
                && timeline_session_messages.last().is_none_or(|message| {
                    message.resume_cursor.as_deref() != resume_cursor.as_deref()
                })
            {
                timeline_session_messages.push(
                    crate::modules::runtime::session::ConversationMessage {
            ...
            let candidates_stream = extract_memory_store_tool_candidates(
                &new_messages_stream,
            ...
            // mutates the store on its own.
            {
                ...
                match engine.evaluate_and_audit().await {
                    ...
                    Ok(Some(_)) => {
                        tracing::debug!(
                            "[start_agent_stream] PromotionEngine: scan ran, no candidates"
                        )
                    }
                    Ok(None) => {
                        tracing::debug!(
                            "[start_agent_stream] PromotionEngine: throttled, scan skipped"
                        )
                    }
                    Err(e) => {
